...
def test_cli_nonexistent_extended_states_file(tmp_path):
    ...
    result = subprocess.run(['python3', 'get_manual_pkgs.py', 
                             '--extended', str(nonexistent_extended)],
                             capture_output=True, text=True)
    assert result.returncode == 1
...
def test_cli_nonexistent_history_log_files(tmp_path):
    ...
    result = subprocess.run(['python3', 'get_manual_pkgs.py', 
                             '--history-logs', str(nonexistent_history)],
                             capture_output=True, text=True)
    assert result.returncode == 1
    assert f"Error: Log file not found: {nonexistent_history}" in result.stdout

def test_cli_no_packages_found(no_packages_installed_status_file, normal_extended_states_file, normal_history_log_file, normal_history_log_file_2):
    result = subprocess.run(['python3', 'get_manual_pkgs.py', 
                             '--status', str(no_packages_installed_status_file), 
                             '--extended', str(normal_extended_states_file), 
                             '--history-logs', str(normal_history_log_file), str(normal_history_log_file_2)],
                            capture_output=True, text=True)
    assert result.returncode == 0
...
def test_cli_output_to_file(normal_status_file, normal_extended_states_file, normal_history_log_file, normal_history_log_file_2, tmp_path):
