        ...
        with open(path) as file:
            for line in file:
                ...
                if (
                    ...
                    and "install" in line
                    and ("apt " in line or "apt-get " in line)
                ):
                    ...
                    current_command_line = [
                        arg for arg in parts[install_index + 1 :] if not arg.startswith("-")
                    ]
...
def get_manual_packages(status_path, extended_path, log_paths, verbose):
    ...
    if verbose:
        ...
        print(
            f"{BLUE_TEXT}Manual packages from extended states: {len(manual_extended)}{RESET_TEXT_COLOR}"
        )
        print(
            f"{BLUE_TEXT}Manual packages from history logs: {len(manual_history)}{RESET_TEXT_COLOR}"
        )
        print(
            f"{BLUE_TEXT}Final list of manually installed packages found: {len(manual_packages)}{RESET_TEXT_COLOR}"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "List packages explicitly installed by the user.\n"
            "Examples:\n"
            "  manual-pkgs -v\n"
            "  manual-pkgs --status /var/lib/dpkg/status\n"
            "  manual-pkgs --output packages.txt --history-logs /var/log/apt/history.log"
    ...
    parser.add_argument(
        ...
        help=(
            "Path to the apt extended_states file.\n"
            "Example:\n"
    ...
    parser.add_argument(
        ...
        help=(
            "Paths to the apt history log files to check.\n"
            "Example:\n"
    ...
    manual_packages = sorted(
        get_manual_packages(args.status, args.extended, args.history_logs, args.verbose)
    )

    if not manual_packages:
        print(
            f"{BLUE_TEXT}No packages found that were explicitly installed by the user.{RESET_TEXT_COLOR}"
        )
    else:
        if args.output:
            with open(args.output, "w") as file:
                ...
                for package in manual_packages:
                    file.write(package + "\n")
                print(
                    f"{GREEN_TEXT}Saved list of packages explicitly installed by the user to {args.output}{RESET_TEXT_COLOR}"
                )
        else:
